...
\begin{code}%
\>[0]\AgdaComment{--\ §\ Bridge\ code\ (moved\ from\ Void.lagda.tex)\ —\ All\ correction\ chain\ definitions:\ loop\ numerator,\ proton,\ EW,\ muon,}\<%
\\
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ The\ universal\ loop\ numerator:\ sum\ of\ all\ primitive\ loop\ parameters.}\<%
\\
\>[0]\AgdaFunction{loop-numerator}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaDatatype{ℕ}\<%
\\
\>[0]\AgdaFunction{loop-numerator}\AgdaSpace{}%
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Tree-level\ proton-to-electron\ mass\ ratio\ from\ K₄\ invariants}\<%
\\
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Law:\ proton\ bare\ mass\ is\ exactly\ 1836}\<%
\\
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Alternative\ factorization:\ d\ ×\ E²\ ×\ F₂}\<%
\\
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Both\ factorizations\ agree}\<%
\\
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ The\ identity\ that\ connects\ them:\ χ\ ·\ d\ =\ E}\<%
\\
\>[0]\AgdaFunction{law-chi-times-d-is-E}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{eulerChar-computed}\AgdaSpace{}%
\AgdaOperator{\AgdaPrimitive{*}}\AgdaSpace{}%
\AgdaFunction{degree-K4}\AgdaSpace{}%
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Tree-level\ muon-to-electron\ mass\ ratio\ from\ K₄\ invariants}\<%
\\
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Law:\ muon\ bare\ mass\ ratio\ is\ exactly\ 207}\<%
\\
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Tau-to-muon\ ratio:\ the\ Fermat\ stratum\ alone}\<%
\\
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Law:\ tau/muon\ bare\ ratio\ is\ 17}\<%
\\
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Law:\ loop\ numerator\ is\ exactly\ 11}\<%
\\
\>[0]\AgdaFunction{law-loop-num-11}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Decomposition:\ the\ three\ structural\ contributions}\<%
\\
\>[0]\AgdaFunction{law-loop-num-decomposition}\AgdaSpace{}%
\AgdaSymbol{:}\<%
\\
\>[0][@{}l@{\AgdaIndent{0}}]%
\>[2]\AgdaFunction{loop-numerator}\AgdaSpace{}%
\AgdaOperator{\AgdaDatatype{≡}}\AgdaSpace{}%
...
The universal loop numerator $11 = E + d + \chi$ will now be subjected to
an exhaustive classification.  Its denominators---$72$ at the QCD
scale, $576$ at the electroweak scale---are the canonical volumes of
the graph, and their ratio is exactly $\kappa = 8$.

\begin{code}%
\>[0]\AgdaComment{--\ §\ Denominator\ definitions\ for\ the\ classification\ proof.}\<%
\\
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Loop\ denominator\ at\ QCD\ (hadron)\ scale}\<%
\\
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Loop\ denominator\ at\ electroweak\ scale}\<%
\\
...
%───────────────────────────────────────────────────────────────────────────
\subsubsection*{The Eight Candidates}
%───────────────────────────────────────────────────────────────────────────

The graph $K_4$ supplies three primitive loop parameters:
edge count $E = 6$, degree $d = 3$, and Euler characteristic $\chi = 2$.
Any additive loop numerator built from these three must be a
$\{0,1\}$-linear combination---a subset sum---because each generator
enters a loop integral exactly once or not at all.
There are $2^3 = 8$ such combinations.
The QCD denominator is $72 = V \cdot E \cdot d = 4 \cdot 6 \cdot 3$,
the electroweak denominator is $576 = \kappa \cdot 72$.
A correction fraction $p/q$ is physically admissible only if
$\gcd(p,q) = 1$; any common factor would indicate a reducible
decomposition, contradicting the atomicity of the loop.

The eight candidates and their values:
\[
\begin{array}{c|cccccccc}
\text{subset} & \varnothing & \chi & d & d{+}\chi & E & E{+}\chi & E{+}d & E{+}d{+}\chi \\
\hline
\text{value}  & 0 & 2 & 3 & 5 & 6 & 8 & 9 & 11
\end{array}
\]
Filter~1 (irreducibility with $72$) eliminates six of these.
Filter~2 (structural completeness) eliminates the seventh.
Only $11 = E + d + \chi$ survives.

\begin{code}%
\>[0]\AgdaComment{--\ §\ Exhaustive\ loop\ classification\ over\ the\ 2³\ \{0,1\}-linear\ combinations\ of\ \{E,\ d,\ χ\}.}\<%
\\
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ The\ eight\ candidates\ and\ their\ values:}\<%
\\
\>[0]\AgdaFunction{loop-cand-000}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaDatatype{ℕ}%
\>[28]\AgdaComment{--\ empty\ set}\<%
\\
\>[0]\AgdaFunction{loop-cand-000}\AgdaSpace{}%
\AgdaSymbol{=}\AgdaSpace{}%
\AgdaNumber{0}\<%
\\
\>[0]\AgdaFunction{loop-cand-001}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaDatatype{ℕ}%
\>[28]\AgdaComment{--\ χ\ only}\<%
\\
\>[0]\AgdaFunction{loop-cand-001}\AgdaSpace{}%
\AgdaSymbol{=}\AgdaSpace{}%
\AgdaFunction{eulerChar-computed}\<%
\\
\>[0]\AgdaFunction{loop-cand-010}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaDatatype{ℕ}%
\>[28]\AgdaComment{--\ d\ only}\<%
\\
\>[0]\AgdaFunction{loop-cand-010}\AgdaSpace{}%
\AgdaSymbol{=}\AgdaSpace{}%
\AgdaFunction{degree-K4}\<%
\\
\>[0]\AgdaFunction{loop-cand-011}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaDatatype{ℕ}%
\>[28]\AgdaComment{--\ d\ +\ χ}\<%
\\
...
\AgdaFunction{eulerChar-computed}\<%
\\
\>[0]\AgdaFunction{loop-cand-100}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaDatatype{ℕ}%
\>[28]\AgdaComment{--\ E\ only}\<%
\\
\>[0]\AgdaFunction{loop-cand-100}\AgdaSpace{}%
\AgdaSymbol{=}\AgdaSpace{}%
\AgdaFunction{edgeCountK4}\<%
\\
\>[0]\AgdaFunction{loop-cand-101}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaDatatype{ℕ}%
\>[28]\AgdaComment{--\ E\ +\ χ}\<%
\\
...
\AgdaFunction{eulerChar-computed}\<%
\\
\>[0]\AgdaFunction{loop-cand-110}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaDatatype{ℕ}%
\>[28]\AgdaComment{--\ E\ +\ d}\<%
\\
...
\AgdaFunction{degree-K4}\<%
\\
\>[0]\AgdaFunction{loop-cand-111}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaDatatype{ℕ}%
\>[28]\AgdaComment{--\ E\ +\ d\ +\ χ}\<%
\\
\>[0]\AgdaFunction{loop-cand-111}\AgdaSpace{}%
\AgdaSymbol{=}\AgdaSpace{}%
\AgdaFunction{edgeCountK4}\AgdaSpace{}%
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Values}\<%
\\
\>[0]\AgdaFunction{law-cand-000}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{loop-cand-000}\AgdaSpace{}%
\AgdaOperator{\AgdaDatatype{≡}}\AgdaSpace{}%
\AgdaNumber{0}\<%
\\
\>[0]\AgdaFunction{law-cand-000}\AgdaSpace{}%
...
\AgdaInductiveConstructor{refl}\<%
\\
\>[0]\AgdaFunction{law-cand-001}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{loop-cand-001}\AgdaSpace{}%
\AgdaOperator{\AgdaDatatype{≡}}\AgdaSpace{}%
\AgdaNumber{2}\<%
\\
\>[0]\AgdaFunction{law-cand-001}\AgdaSpace{}%
...
\AgdaInductiveConstructor{refl}\<%
\\
\>[0]\AgdaFunction{law-cand-010}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{loop-cand-010}\AgdaSpace{}%
\AgdaOperator{\AgdaDatatype{≡}}\AgdaSpace{}%
\AgdaNumber{3}\<%
\\
\>[0]\AgdaFunction{law-cand-010}\AgdaSpace{}%
...
\AgdaInductiveConstructor{refl}\<%
\\
\>[0]\AgdaFunction{law-cand-011}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{loop-cand-011}\AgdaSpace{}%
\AgdaOperator{\AgdaDatatype{≡}}\AgdaSpace{}%
\AgdaNumber{5}\<%
\\
\>[0]\AgdaFunction{law-cand-011}\AgdaSpace{}%
...
\AgdaInductiveConstructor{refl}\<%
\\
\>[0]\AgdaFunction{law-cand-100}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{loop-cand-100}\AgdaSpace{}%
\AgdaOperator{\AgdaDatatype{≡}}\AgdaSpace{}%
\AgdaNumber{6}\<%
\\
\>[0]\AgdaFunction{law-cand-100}\AgdaSpace{}%
...
\AgdaInductiveConstructor{refl}\<%
\\
\>[0]\AgdaFunction{law-cand-101}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{loop-cand-101}\AgdaSpace{}%
\AgdaOperator{\AgdaDatatype{≡}}\AgdaSpace{}%
\AgdaNumber{8}\<%
\\
\>[0]\AgdaFunction{law-cand-101}\AgdaSpace{}%
...
\AgdaInductiveConstructor{refl}\<%
\\
\>[0]\AgdaFunction{law-cand-110}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{loop-cand-110}\AgdaSpace{}%
\AgdaOperator{\AgdaDatatype{≡}}\AgdaSpace{}%
\AgdaNumber{9}\<%
\\
\>[0]\AgdaFunction{law-cand-110}\AgdaSpace{}%
...
\AgdaInductiveConstructor{refl}\<%
\\
\>[0]\AgdaFunction{law-cand-111}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{loop-cand-111}\AgdaSpace{}%
\AgdaOperator{\AgdaDatatype{≡}}\AgdaSpace{}%
\AgdaNumber{11}\<%
\\
\>[0]\AgdaFunction{law-cand-111}\AgdaSpace{}%
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Filter\ 1:\ irreducibility\ with\ 72\ eliminates\ 0,\ 2,\ 3,\ 6,\ 8,\ and\ 9.}\<%
\\
\>[0]\AgdaFunction{law-elim-000}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
...
\AgdaOperator{\AgdaDatatype{≡}}\AgdaSpace{}%
\AgdaNumber{2}%
\>[54]\AgdaComment{--\ χ=2\ shares\ factor\ 2}\<%
\\
\>[0]\AgdaFunction{law-elim-000}\AgdaSpace{}%
\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}\<%
\\
\>[0]\AgdaFunction{law-elim-010}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{gcd}\AgdaSpace{}%
\AgdaFunction{loop-cand-010}\AgdaSpace{}%
\AgdaFunction{loop-denom-QCD}\AgdaSpace{}%
\AgdaOperator{\AgdaDatatype{≡}}\AgdaSpace{}%
\AgdaNumber{3}%
\>[54]\AgdaComment{--\ d=3\ shares\ factor\ 3}\<%
\\
\>[0]\AgdaFunction{law-elim-010}\AgdaSpace{}%
\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}\<%
\\
\>[0]\AgdaFunction{law-elim-100}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{gcd}\AgdaSpace{}%
\AgdaFunction{loop-cand-100}\AgdaSpace{}%
\AgdaFunction{loop-denom-QCD}\AgdaSpace{}%
\AgdaOperator{\AgdaDatatype{≡}}\AgdaSpace{}%
\AgdaNumber{6}%
\>[54]\AgdaComment{--\ E=6\ shares\ factor\ 6}\<%
\\
\>[0]\AgdaFunction{law-elim-100}\AgdaSpace{}%
\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}\<%
\\
\>[0]\AgdaFunction{law-elim-101}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{gcd}\AgdaSpace{}%
\AgdaFunction{loop-cand-101}\AgdaSpace{}%
\AgdaFunction{loop-denom-QCD}\AgdaSpace{}%
\AgdaOperator{\AgdaDatatype{≡}}\AgdaSpace{}%
\AgdaNumber{8}%
\>[54]\AgdaComment{--\ E+χ=8\ shares\ factor\ 8}\<%
\\
\>[0]\AgdaFunction{law-elim-101}\AgdaSpace{}%
\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}\<%
\\
\>[0]\AgdaFunction{law-elim-110}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{gcd}\AgdaSpace{}%
\AgdaFunction{loop-cand-110}\AgdaSpace{}%
\AgdaFunction{loop-denom-QCD}\AgdaSpace{}%
\AgdaOperator{\AgdaDatatype{≡}}\AgdaSpace{}%
\AgdaNumber{9}%
\>[54]\AgdaComment{--\ E+d=9\ shares\ factor\ 9}\<%
\\
\>[0]\AgdaFunction{law-elim-110}\AgdaSpace{}%
\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}\<%
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Pass\ irreducibility:\ d+χ=5\ and\ E+d+χ=11}\<%
\\
\>[0]\AgdaFunction{law-pass-011}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{gcd}\AgdaSpace{}%
\AgdaFunction{loop-cand-011}\AgdaSpace{}%
\AgdaFunction{loop-denom-QCD}\AgdaSpace{}%
\AgdaOperator{\AgdaDatatype{≡}}\AgdaSpace{}%
\AgdaNumber{1}%
\>[54]\AgdaComment{--\ d+χ=5\ coprime\ ✓}\<%
\\
\>[0]\AgdaFunction{law-pass-011}\AgdaSpace{}%
\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}\<%
\\
\>[0]\AgdaFunction{law-pass-111}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{gcd}\AgdaSpace{}%
\AgdaFunction{loop-cand-111}\AgdaSpace{}%
\AgdaFunction{loop-denom-QCD}\AgdaSpace{}%
\AgdaOperator{\AgdaDatatype{≡}}\AgdaSpace{}%
\AgdaNumber{1}%
\>[54]\AgdaComment{--\ E+d+χ=11\ coprime\ ✓}\<%
\\
\>[0]\AgdaFunction{law-pass-111}\AgdaSpace{}%
\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}\<%
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Filter\ 2:\ completeness\ requires\ all\ three\ basis\ elements,\ so\ d+χ=5\ is\ missing\ the\ E-sector.}\<%
\\
\>[0]\AgdaFunction{law-partial-omits-E}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{loop-cand-011}\AgdaSpace{}%
\AgdaOperator{\AgdaPrimitive{+}}\AgdaSpace{}%
\AgdaFunction{edgeCountK4}\AgdaSpace{}%
\AgdaOperator{\AgdaDatatype{≡}}\AgdaSpace{}%
\AgdaFunction{loop-cand-111}\<%
\\
\>[0]\AgdaFunction{law-partial-omits-E}\AgdaSpace{}%
\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}\<%
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Equivalently:\ the\ full\ candidate\ is\ the\ partial\ plus\ the\ missing\ E-sector.}\<%
\\
\>[0]\AgdaFunction{law-full-is-partial-plus-E}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{loop-cand-111}\AgdaSpace{}%
\AgdaOperator{\AgdaDatatype{≡}}\AgdaSpace{}%
\AgdaFunction{edgeCountK4}\AgdaSpace{}%
\AgdaOperator{\AgdaPrimitive{+}}\AgdaSpace{}%
\AgdaFunction{loop-cand-011}\<%
\\
\>[0]\AgdaFunction{law-full-is-partial-plus-E}\AgdaSpace{}%
\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}\<%
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ After\ Filter\ 2:\ d+χ=5\ is\ eliminated.\ Only\ E+d+χ=11\ remains.}\<%
\\
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Structural\ independence:\ E\ is\ multiplicatively\ independent\ of\ d+χ,\ so\ the\ edge\ stratum\ cannot\ be\ recovered\ by\ rescaling.}\<%
\\
\>[0]\AgdaFunction{law-E-structurally-independent}\AgdaSpace{}%
\AgdaSymbol{:}\<%
\\
\>[0][@{}l@{\AgdaIndent{0}}]%
...
\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}%
\>[39]\AgdaComment{--\ gcd(6,\ 5)\ =\ 1}\<%
\\
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ E\ =\ 6\ spans\ the\ full\ \{2,3\}\ prime\ base\ inside\ the\ loop\ volume\ 72\ =\ 2³·3².}\<%
\\
\>[0]\AgdaFunction{law-E-spans-full-base}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{gcd}\AgdaSpace{}%
\AgdaFunction{edgeCountK4}\AgdaSpace{}%
...
\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}%
\>[30]\AgdaComment{--\ gcd(6,\ 6)\ =\ 6}\<%
\\
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ d\ =\ 3\ contributes\ only\ the\ 3-stratum}\<%
\\
\>[0]\AgdaFunction{law-d-in-3-stratum}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{gcd}\AgdaSpace{}%
\AgdaFunction{degree-K4}\AgdaSpace{}%
\AgdaSymbol{(}\AgdaFunction{eulerChar-computed}\AgdaSpace{}%
...
\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}%
\>[27]\AgdaComment{--\ gcd(3,\ 6)\ =\ 3}\<%
\\
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ χ\ =\ 2\ contributes\ only\ the\ 2-stratum}\<%
\\
\>[0]\AgdaFunction{law-chi-in-2-stratum}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{gcd}\AgdaSpace{}%
\AgdaFunction{eulerChar-computed}\AgdaSpace{}%
\AgdaSymbol{(}\AgdaFunction{eulerChar-computed}\AgdaSpace{}%
...
\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}%
\>[29]\AgdaComment{--\ gcd(2,\ 6)\ =\ 2}\<%
\\
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ The\ partial\ sum\ d+χ\ =\ 5\ escapes\ the\ \{2,3\}\ stratum,\ leaving\ the\ E-stratum\ unaccounted\ for;\ only\ E+d+χ\ =\ 11\ covers\ all\ three.}\<%
\\
\>[0]\AgdaFunction{law-dchi-escapes-stratum}\AgdaSpace{}%
\AgdaSymbol{:}\<%
\\
\>[0][@{}l@{\AgdaIndent{0}}]%
\>[2]\AgdaFunction{gcd}\AgdaSpace{}%
...
\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}%
\>[33]\AgdaComment{--\ gcd(5,\ 6)\ =\ 1}\<%
\end{code}

%───────────────────────────────────────────────────────────────────────────
\subsubsection*{Why $d + \chi = 5$ Fails: The Completeness Argument}
%───────────────────────────────────────────────────────────────────────────

After Filter~1, two candidates survive irreducibility:
$d + \chi = 5$ and $E + d + \chi = 11$.
Both are coprime to $72$.  A na\"ive counting argument would declare
the classification ambiguous, but the prime-stratum structure of the
denominator shatters this illusion.

The QCD volume $72 = 2^3 \cdot 3^2$ has two independent prime strata.
The edge count $E = 6 = 2 \cdot 3$ spans \emph{both},
$d = 3$ covers only the $3$-stratum, and $\chi = 2$ covers only the
$2$-stratum.  The sum $d + \chi = 5$ is coprime to $E = 6$ and to
$\chi \cdot d = 6$---it escapes every prime stratum of the volume.
A numerator that is structurally blind to the full factorisation of its
own denominator cannot represent a complete loop correction.

The identity $\chi \cdot d = E$ forces the missing link: the edge
stratum is not an independent generator but the product of the other
two.  Adding $E$ to $d + \chi$ recovers the full witness
$E + d + \chi = 11$, which contains contributions from every prime
factor of $72$.  The partial sum $5$ omits $E$ and can never recover
it---$\gcd(5, 6) = 1$ proves that $5$ and $6$ share no prime structure
whatsoever.

\textbf{Constraint:}
The loop correction fraction $p/q$ must be irreducible \emph{and} its
numerator must witness all prime strata of $q$.
A numerator coprime to $q$ but blind to a prime factor of $q$
represents an incomplete integration---it misses an entire sector of
the graph.

\textbf{Elimination:}
$d + \chi = 5$ passes irreducibility but fails completeness:
$\gcd(5, 6) = 1$ proves it shares no structure with the edge stratum.
Only $E + d + \chi = 11$ survives both filters.

\textbf{Consequence:}
The unique loop numerator is $11$.  It decomposes as $6 + 3 + 2$,
witnessing every generator of $K_4$ that enters the loop integral.
No alternative exists.

\begin{code}%
\>[0]\AgdaComment{--\ §\ Completeness\ derivation\ from\ χ·d\ =\ E\ and\ the\ interaction-volume\ structure.}\<%
\\
\>[0]\AgdaKeyword{record}\AgdaSpace{}%
\AgdaRecord{CompletenessDerivation}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaPrimitive{Set}\AgdaSpace{}%
\AgdaKeyword{where}\<%
\\
...
\\
\>[2][@{}l@{\AgdaIndent{0}}]%
\>[4]\AgdaComment{--\ Step\ 1:\ the\ multiplicative\ relation\ links\ all\ three\ generators}\<%
\\
%
\>[4]\AgdaField{relation}%
...
\\
%
\>[4]\AgdaComment{--\ The\ two\ generators\ χ\ and\ d\ are\ coprime}\<%
\\
%
\>[4]\AgdaField{generators-coprime}%
\>[24]\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{gcd}\AgdaSpace{}%
\AgdaFunction{eulerChar-computed}\AgdaSpace{}%
\AgdaFunction{degree-K4}\AgdaSpace{}%
...
\\
%
\>[4]\AgdaComment{--\ Step\ 2:\ the\ volume\ rewrites\ via\ χ·d\ =\ E\ as\ D\ =\ V·χ·d²}\<%
\\
%
\>[4]\AgdaField{volume-via-relation}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaSymbol{(}\AgdaFunction{vertexCountK4}\AgdaSpace{}%
\AgdaOperator{\AgdaPrimitive{*}}\AgdaSpace{}%
\AgdaFunction{eulerChar-computed}\AgdaSymbol{)}\AgdaSpace{}%
...
\\
%
\>[4]\AgdaComment{--\ Step\ 3:\ sum\ and\ product\ of\ coprime\ generators\ are\ coprime}\<%
\\
%
\>[4]\AgdaField{sum-product-coprime}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{gcd}\AgdaSpace{}%
\AgdaSymbol{(}\AgdaFunction{degree-K4}\AgdaSpace{}%
\AgdaOperator{\AgdaPrimitive{+}}\AgdaSpace{}%
...
\\
%
\>[4]\AgdaComment{--\ The\ partial\ numerator\ d+χ\ is\ coprime\ to\ E\ —\ structurally\ disconnected}\<%
\\
%
\>[4]\AgdaField{partial-blind-to-E}%
\>[24]\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{gcd}\AgdaSpace{}%
\AgdaFunction{loop-cand-011}\AgdaSpace{}%
\AgdaFunction{edgeCountK4}\AgdaSpace{}%
...
\\
%
\>[4]\AgdaComment{--\ The\ full\ sum\ includes\ E\ alongside\ d+χ}\<%
\\
%
\>[4]\AgdaField{full-witnesses-E}%
\>[24]\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{loop-cand-111}\AgdaSpace{}%
\AgdaOperator{\AgdaDatatype{≡}}\AgdaSpace{}%
\AgdaFunction{edgeCountK4}\AgdaSpace{}%
...
\\
%
\>[4]\AgdaComment{--\ The\ full\ sum\ passes\ irreducibility}\<%
\\
%
\>[4]\AgdaField{full-irreducible}%
\>[24]\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{gcd}\AgdaSpace{}%
\AgdaFunction{loop-cand-111}\AgdaSpace{}%
\AgdaFunction{loop-denom-QCD}\AgdaSpace{}%
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaFunction{theorem-completeness-derivation}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaRecord{CompletenessDerivation}\<%
\\
\>[0]\AgdaFunction{theorem-completeness-derivation}\AgdaSpace{}%
\AgdaSymbol{=}\AgdaSpace{}%
\AgdaKeyword{record}\<%
...
%
\>[2]\AgdaSymbol{;}\AgdaSpace{}%
\AgdaField{generators-coprime}%
\>[24]\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}\<%
\\
%
\>[2]\AgdaSymbol{;}\AgdaSpace{}%
\AgdaField{volume-via-relation}\AgdaSpace{}%
\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}\<%
\\
%
\>[2]\AgdaSymbol{;}\AgdaSpace{}%
\AgdaField{sum-product-coprime}\AgdaSpace{}%
\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}\<%
\\
%
\>[2]\AgdaSymbol{;}\AgdaSpace{}%
\AgdaField{partial-blind-to-E}%
\>[24]\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}\<%
\\
%
\>[2]\AgdaSymbol{;}\AgdaSpace{}%
\AgdaField{full-witnesses-E}%
\>[24]\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}\<%
\\
%
\>[2]\AgdaSymbol{;}\AgdaSpace{}%
\AgdaField{full-irreducible}%
\>[24]\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}\<%
\\
%
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Filter\ 3:\ cross-scale\ consistency\ confirms\ that\ the\ same\ observable\ 11\ survives\ at\ EW\ scale;\ it\ does\ not\ newly\ eliminate\ d+χ.}\<%
\\
\>[0]\AgdaFunction{law-partial-EW}%
...
\AgdaOperator{\AgdaDatatype{≡}}\AgdaSpace{}%
\AgdaNumber{1}%
\>[56]\AgdaComment{--\ 5\ coprime\ to\ 576\ too}\<%
\\
\>[0]\AgdaFunction{law-partial-EW}%
\>[16]\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}\<%
\\
\>[0]\AgdaFunction{law-pass-111-EW}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{gcd}\AgdaSpace{}%
\AgdaFunction{loop-cand-111}\AgdaSpace{}%
\AgdaFunction{loop-denom-EW}\AgdaSpace{}%
\AgdaOperator{\AgdaDatatype{≡}}\AgdaSpace{}%
\AgdaNumber{1}%
\>[56]\AgdaComment{--\ 11\ coprime\ to\ 576\ ✓}\<%
\\
\>[0]\AgdaFunction{law-pass-111-EW}\AgdaSpace{}%
\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}\<%
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Loop\ classification\ record\ certifying\ the\ unique\ admissible\ loop\ observable.}\<%
\\
\>[0]\AgdaKeyword{record}\AgdaSpace{}%
\AgdaRecord{LoopClassification}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaPrimitive{Set}\AgdaSpace{}%
\AgdaKeyword{where}\<%
\\
...
\\
\>[2][@{}l@{\AgdaIndent{0}}]%
\>[4]\AgdaComment{--\ The\ forced\ value}\<%
\\
%
\>[4]\AgdaField{forced-value}%
\>[24]\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{loop-numerator}\AgdaSpace{}%
\AgdaOperator{\AgdaDatatype{≡}}\AgdaSpace{}%
\AgdaNumber{11}\<%
...
\\[\AgdaEmptyExtraSkip]%
%
\>[4]\AgdaComment{--\ Irreducibility\ at\ both\ scales}\<%
\\
%
\>[4]\AgdaField{irreducible-QCD}%
\>[24]\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{gcd}\AgdaSpace{}%
\AgdaFunction{loop-numerator}\AgdaSpace{}%
\AgdaFunction{loop-denom-QCD}\AgdaSpace{}%
...
\\[\AgdaEmptyExtraSkip]%
%
\>[4]\AgdaComment{--\ All\ other\ candidates\ eliminated\ by\ irreducibility}\<%
\\
%
\>[4]\AgdaField{elim-chi}%
\>[24]\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{gcd}\AgdaSpace{}%
\AgdaFunction{eulerChar-computed}\AgdaSpace{}%
\AgdaFunction{loop-denom-QCD}\AgdaSpace{}%
...
\\[\AgdaEmptyExtraSkip]%
%
\>[4]\AgdaComment{--\ The\ only\ other\ coprime\ candidate\ (d+χ=5)\ fails\ completeness}\<%
\\
%
\>[4]\AgdaField{partial-coprime}%
\>[24]\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{gcd}\AgdaSpace{}%
\AgdaSymbol{(}\AgdaFunction{degree-K4}\AgdaSpace{}%
\AgdaOperator{\AgdaPrimitive{+}}\AgdaSpace{}%
...
\\
%
\>[4]\AgdaField{partial-incomplete}%
\>[24]\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{degree-K4}\AgdaSpace{}%
\AgdaOperator{\AgdaPrimitive{+}}\AgdaSpace{}%
\AgdaFunction{eulerChar-computed}\AgdaSpace{}%
\AgdaOperator{\AgdaDatatype{≡}}\AgdaSpace{}%
\AgdaNumber{5}%
\>[62]\AgdaComment{--\ omits\ E}\<%
\\
%
\>[4]\AgdaComment{--\ Formal\ completeness\ witness:\ the\ missing\ contribution\ is\ exactly\ E}\<%
\\
%
\>[4]\AgdaField{partial-omits-E}%
\>[24]\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{loop-cand-011}\AgdaSpace{}%
\AgdaOperator{\AgdaPrimitive{+}}\AgdaSpace{}%
\AgdaFunction{edgeCountK4}\AgdaSpace{}%
...
\\[\AgdaEmptyExtraSkip]%
%
\>[4]\AgdaComment{--\ Cross-scale\ confirmation:\ 5\ also\ passes\ EW,\ so\ Filter\ 3\ is\ not\ the\ eliminator}\<%
\\
%
\>[4]\AgdaField{partial-also-EW}%
\>[24]\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{gcd}\AgdaSpace{}%
\AgdaFunction{loop-cand-011}\AgdaSpace{}%
\AgdaFunction{loop-denom-EW}\AgdaSpace{}%
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Proof:\ every\ alternative\ is\ eliminated}\<%
\\
\>[0]\AgdaFunction{theorem-loop-classification}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaRecord{LoopClassification}\<%
\\
\>[0]\AgdaFunction{theorem-loop-classification}\AgdaSpace{}%
\AgdaSymbol{=}\AgdaSpace{}%
\AgdaKeyword{record}\<%
...
\>[0][@{}l@{\AgdaIndent{0}}]%
\>[2]\AgdaSymbol{\{}\AgdaSpace{}%
\AgdaField{forced-value}%
\>[24]\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}\<%
\\
%
\>[2]\AgdaSymbol{;}\AgdaSpace{}%
\AgdaField{forced-from-K4}%
\>[24]\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}\<%
\\
%
\>[2]\AgdaSymbol{;}\AgdaSpace{}%
\AgdaField{irreducible-QCD}%
\>[24]\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}\<%
\\
%
\>[2]\AgdaSymbol{;}\AgdaSpace{}%
\AgdaField{irreducible-EW}%
\>[24]\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}\<%
\\
%
...
%
\>[2]\AgdaSymbol{;}\AgdaSpace{}%
\AgdaField{partial-coprime}%
\>[24]\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}\<%
\\
%
\>[2]\AgdaSymbol{;}\AgdaSpace{}%
\AgdaField{partial-incomplete}%
\>[24]\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}\<%
\\
%
\>[2]\AgdaSymbol{;}\AgdaSpace{}%
\AgdaField{partial-omits-E}%
\>[24]\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}\<%
\\
%
\>[2]\AgdaSymbol{;}\AgdaSpace{}%
\AgdaField{partial-also-EW}%
\>[24]\AgdaSymbol{=}\AgdaSpace{}%
\AgdaInductiveConstructor{refl}\<%
\\
%
...
\>[0][@{}l@{\AgdaIndent{0}}]%
\>[2]\AgdaSymbol{\{}%
\>[3665I]\AgdaField{unique-loop-numerator}\AgdaSpace{}%
\AgdaSymbol{=}\AgdaSpace{}%
\AgdaSymbol{λ}\AgdaSpace{}%
\AgdaKeyword{where}\<%
\\
\>[3665I][@{}l@{\AgdaIndent{0}}]%
...
%
\>[2]\AgdaSymbol{;}%
\>[4103I]\AgdaField{admissible}\AgdaSpace{}%
\AgdaSymbol{=}\AgdaSpace{}%
...
%
\>[2]\AgdaSymbol{;}%
\>[4120I]\AgdaField{above-cutoff-is-iterated}\AgdaSpace{}%
\AgdaSymbol{=}\<%
\\
\>[4120I][@{}l@{\AgdaIndent{0}}]%
...
%
\>[2]\AgdaSymbol{;}%
\>[4156I]\AgdaField{quartic-chi-is-not-primitive}\AgdaSpace{}%
\AgdaSymbol{=}\<%
\\
\>[4156I][@{}l@{\AgdaIndent{0}}]%
\>[6]\AgdaSymbol{λ}%
\>[4158I]\AgdaBound{adm}\AgdaSpace{}%
\AgdaSymbol{→}\<%
\\
\>[.][@{}l@{}]\<[4158I]%
...
\>[4168I]\AgdaOperator{\AgdaDatatype{≡}}%
...
%
\>[2]\AgdaSymbol{;}%
\>[4573I]\AgdaField{reduced-cofactor-exactly-primitive-108}\AgdaSpace{}%
\AgdaSymbol{=}\<%
\\
\>[4573I][@{}l@{\AgdaIndent{0}}]%
\>[6]\AgdaSymbol{λ}\AgdaSpace{}%
\AgdaBound{m}\AgdaSpace{}%
\AgdaSymbol{→}\AgdaSpace{}%
\AgdaSymbol{(λ}\AgdaSpace{}%
...
%
\>[2]\AgdaSymbol{;}%
\>[4609I]\AgdaField{topological-route-is-iterated}\AgdaSpace{}%
\AgdaSymbol{=}\<%
\\
\>[4609I][@{}l@{\AgdaIndent{0}}]%
\>[6]\AgdaSymbol{λ}%
\>[4611I]\AgdaBound{adm}\AgdaSpace{}%
\AgdaSymbol{→}\<%
\\
\>[.][@{}l@{}]\<[4611I]%
\>[8]\AgdaFunction{primitive-vs-iterated-impossible}\AgdaSpace{}%
...
\AgdaBound{n}\AgdaSpace{}%
\AgdaSymbol{=}%
\>[6954I]\AgdaKeyword{let}%
\>[6955I]\AgdaBound{e}\AgdaSpace{}%
\AgdaSymbol{=}\AgdaSpace{}%
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Continuum\ Limit\ Theorems\ —\ The\ fold\ (catamorphism)\ maps\ discrete\ K₄\ lattice\ →\ continuous\ manifold.}\<%
\\
%
\\[\AgdaEmptyExtraSkip]%
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Number\ system\ emergence}\<%
\\
\>[0]\AgdaKeyword{data}\AgdaSpace{}%
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Black\ Hole\ Entropy\ (Bekenstein-Hawking\ derived)}\<%
\\
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaKeyword{module}\AgdaSpace{}%
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Kappa\ forcing\ chain\ —\ κ\ =\ 2V\ =\ 8\ (from\ Void)}\<%
\\
%
\\[\AgdaEmptyExtraSkip]%
...
\>[0]\AgdaFunction{signatureTrace}\AgdaSpace{}%
\AgdaSymbol{=}%
\>[8296I]\AgdaFunction{minkowskiSignature}\AgdaSpace{}%
\AgdaInductiveConstructor{τ-idx}\AgdaSpace{}%
\AgdaInductiveConstructor{τ-idx}\AgdaSpace{}%
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ §\ Spectral\ gap\ λ₄\ =\ 4\ (the\ non-zero\ Laplacian\ eigenvalue\ of\ K₄)}\<%
\\
...
\AgdaBound{v}\AgdaSpace{}%
\AgdaSymbol{=}%
\>[8606I]\AgdaFunction{spectralRicci}\AgdaSpace{}%
\AgdaBound{v}\AgdaSpace{}%
\AgdaInductiveConstructor{x-idx}\AgdaSpace{}%
\AgdaInductiveConstructor{x-idx}\AgdaSpace{}%
\AgdaOperator{\AgdaFunction{+ℤ}}\<%
...
\>[0][@{}l@{\AgdaIndent{0}}]%
\>[2]\AgdaKeyword{let}%
\>[8707I]\AgdaBound{R-μν}\AgdaSpace{}%
\AgdaSymbol{=}\AgdaSpace{}%
...
\AgdaSymbol{:}%
\>[9038I]\AgdaDatatype{K4Vertex}\AgdaSpace{}%
\AgdaSymbol{→}\AgdaSpace{}%
\AgdaDatatype{SpacetimeIndex}\AgdaSpace{}%
\AgdaSymbol{→}\AgdaSpace{}%
\AgdaDatatype{SpacetimeIndex}\AgdaSpace{}%
...
\AgdaSymbol{:}%
\>[9081I]\AgdaDatatype{K4Vertex}\AgdaSpace{}%
\AgdaSymbol{→}\AgdaSpace{}%
\AgdaDatatype{SpacetimeIndex}\AgdaSpace{}%
\AgdaSymbol{→}\AgdaSpace{}%
\AgdaDatatype{SpacetimeIndex}\AgdaSpace{}%
...
\AgdaSymbol{:}%
\>[9271I]\AgdaSymbol{(}\AgdaBound{v}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaDatatype{K4Vertex}\AgdaSymbol{)}\AgdaSpace{}%
\AgdaSymbol{(}\AgdaBound{μ}\AgdaSpace{}%
\AgdaBound{ν}\AgdaSpace{}%
...
\AgdaSymbol{:}%
\>[9288I]\AgdaSymbol{(}\AgdaBound{v}\AgdaSpace{}%
\AgdaBound{w}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaDatatype{K4Vertex}\AgdaSymbol{)}\AgdaSpace{}%
\AgdaSymbol{(}\AgdaBound{μ}\AgdaSpace{}%
...
\AgdaSymbol{:}%
\>[9312I]\AgdaSymbol{(}\AgdaBound{v}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaDatatype{K4Vertex}\AgdaSymbol{)}\AgdaSpace{}%
\AgdaSymbol{(}\AgdaBound{ρ}\AgdaSpace{}%
\AgdaBound{μ}\AgdaSpace{}%
...
\AgdaSymbol{:}%
\>[9328I]\AgdaSymbol{(}\AgdaBound{v}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaDatatype{K4Vertex}\AgdaSymbol{)}\AgdaSpace{}%
\AgdaSymbol{(}\AgdaBound{ρ}\AgdaSpace{}%
\AgdaBound{σ}\AgdaSpace{}%
...
\AgdaSymbol{:}%
\>[9346I]\AgdaSymbol{(}\AgdaBound{v}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaDatatype{K4Vertex}\AgdaSymbol{)}\AgdaSpace{}%
\AgdaSymbol{(}\AgdaBound{ρ}\AgdaSpace{}%
\AgdaBound{σ}\AgdaSpace{}%
...
\AgdaSymbol{:}%
\>[9364I]\AgdaSymbol{(}\AgdaBound{v}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaDatatype{K4Vertex}\AgdaSymbol{)}\AgdaSpace{}%
\AgdaSymbol{(}\AgdaBound{μ}\AgdaSpace{}%
\AgdaBound{ν}\AgdaSpace{}%
...
\>[22]\AgdaSymbol{:}%
\>[10901I]\AgdaSymbol{(}\AgdaBound{v}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaDatatype{K4Vertex}\AgdaSymbol{)}\AgdaSpace{}%
\AgdaSymbol{(}\AgdaBound{ρ}\AgdaSpace{}%
\AgdaBound{μ}\AgdaSpace{}%
...
\AgdaSymbol{:}%
\>[11293I]\AgdaSymbol{(}\AgdaBound{v}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaDatatype{K4Vertex}\AgdaSymbol{)}\AgdaSpace{}%
\AgdaSymbol{(}\AgdaBound{μ}\AgdaSpace{}%
\AgdaBound{ν}\AgdaSpace{}%
...
\AgdaField{ontology}%
\>[22]\AgdaSymbol{=}\AgdaSpace{}%
\AgdaFunction{theorem-ontological-necessity}\AgdaSpace{}%
\AgdaSymbol{\}}\<%
\\
%
\\[\AgdaEmptyExtraSkip]%

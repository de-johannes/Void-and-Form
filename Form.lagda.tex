...
The fine-structure constant stands one tier above the hadron and electroweak
corrections.  Its denominator is not $72$ or $576$ but $306 = d \cdot E
\cdot F_2$---the product of degree, edge count, and compactification prime.
This ``coupling-compactification volume'' governs the electromagnetic sector
because it is the unique product of factors that have not already been consumed
at lower scales.

Six $K_4$ invariants compete for inclusion: $V$, $E$, $d$, $\chi$,
$\kappa$, $F_2$.  Three structural filters annihilate half of them:
\begin{enumerate}
\item $V = 4$ is already consumed in the proton volume $72 = V \cdot E
      \cdot d$.
\item $\kappa = 8$ is already consumed in the electroweak volume
      $576 = V \cdot E \cdot d \cdot \kappa$.
\item $\chi = 2$ enters the bare coupling $\alpha^{-1} = V^d \chi + d^2$;
      using it again would double-count the Euler characteristic.
\end{enumerate}
The three survivors---$d$, $E$, $F_2$---multiply to $306$.  The admitted factor
pool is then a strict subtype of the six candidates, and the inadmissible
constructors receive absurd patterns.
...
\noindent
\textbf{Elimination:}
$V$, $\kappa$, and $\chi$ are consumed by prior tiers.  The three filters
leave $\{d, E, F_2\}$ as the unique admissible factor set.

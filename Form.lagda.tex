...
-- § Projection sign: TwoOrientation from Void (§ Orientation).
-- orient-swap     = σ = −1: all-bulk, orientation reversed by ∂
-- orient-preserve = σ = +1: has boundary ingredient, orientation preserved
-- Exhaustive: orientation-classify in Void proves exactly two cases exist.

-- § The proton projection: all bulk → orient-swap.
proton-projection : TwoOrientation
proton-projection = orient-swap

-- § The muon projection: has boundary → orient-preserve.
muon-projection : TwoOrientation
muon-projection = orient-preserve
